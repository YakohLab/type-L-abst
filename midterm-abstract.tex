...
Downlink requests are preformed by only the controller, however, uplink requests are done by many terminals.
This can lead to collisions, and unsuccessful uplink requests.

To solve this problem, each terminal will preform a re-request after waiting a random amount of time, like ALOHA.
...
\item \textbf{Evaluation 1}, validated the open enviroment performance of the protocol.
  ...
  Upon unregisteration, we also verified that the unused frames were released.
\item \textbf{Evaluation 2}, validated the multiuser perfomance of the protocol.
  Using two terminals registered to the network, we verified that two terminal could stimulaniously send/receive datum.
\end{itemize}
